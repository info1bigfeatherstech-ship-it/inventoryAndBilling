...
const assertBulkRead = async (bulk, user) => {
  ...
  if (['WH_MANAGER', 'WH_STOCK_LISTER'].includes(user.role)) {
    if (user.warehouseId && bulk.from_warehouse_id === user.warehouseId) return;
    return;
  }
...
const BulkTransferService = {
  async createBulkRequest(data, user) {
    try {
      if (user.role !== 'SUPER_ADMIN' && user.role !== 'SHOP_OWNER') {
        throw new AppError('Only shop owners can create bulk transfer requests', 403, 'FORBIDDEN');
      }
      ...
      const [shop, warehouse] = await Promise.all([
        prisma.shop.findUnique({ where: { shop_id: toShopId }, select: { is_active: true } }),
      ...
      const created = await prisma.$transaction(async (tx) => {
        ...
        return tx.bulkTransferRequest.create({
          data: {
            bulk_request_number: bulkNumber,
            request_type: data.request_type || 'WH_TO_SHOP',
  ...
  async listBulkRequests(filters, user) {
    ...
    if (user.role === 'SUPER_ADMIN') {
      ...
      where.from_warehouse_id = user.warehouseId;
  ...
  async receiveBulkRequest(bulkRequestId, data, user) {
    try {
      if (user.role !== 'SUPER_ADMIN' && user.role !== 'SHOP_OWNER') {
        throw new AppError('Only shop owners can receive bulk transfers', 403, 'FORBIDDEN');
      }
